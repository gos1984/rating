...
$(document).ready(function() {
	...
	var timer = setInterval(function() {
		var date = new Residue(2019, 03, 30, 15);
...
$('body').on('change', $('input[data-dependency]'), function(e) {

	var dependency = $(e.target).attr('data-dependency'),
	inputGrid = $(e.target).parents('.quest').find('input');
	var count = 0;
	$.each($(e.target).parents('.quest').find('input:checked'), function(i, el) {
		if($(el).attr('data-dependency') == dependency) {
			count++;
		}
	});
	$.each(inputGrid, function(i, el) {
		if($(e.target).is(':checked')) {
			if($(el).attr('data-dependency') !== dependency) {
				$(el).attr('disabled', true);
				$(el).parents('label').addClass('hidden');
			}
		} else {
			if(count === 0) {
				$(el).attr('disabled', false);
				$(el).parents('label').removeClass('hidden');
			}
		}
	});
	...
	$.ajax({
		...
		success: function(data) {
			...
			function questBuild(q, quest) {
				var questions = '';
				$.each(quest, function(i, el) {
					questions += '<div class="quest">';
					questions += '<p>' + el.name + '</p>';
					$.each(el.answers, function(a, answer) {
						questions += '<label for="ask_' + a + '" ' + (["checkbox","radio"].indexOf(answer.type) !== -1 ? 'class="check ' + answer.type + '"' : '')+ '>';
						questions += '<i class="before"></i><i class="after"></i>';
						questions += '<input id="ask_' + a + '" type="' + answer.type + '" name="quest[' + i + ']' + (answer.type === 'checkbox' ? '[]' : '') + '" value="' + (answer.type === 'checkbox' || answer.type === 'radio' ? a : '') + '" ' + (answer.manager !== null ? 'data-manager="' + answer.manager + '"' : (answer.cond !== null ? 'data-cond="' + answer.cond + '" disabled' : '')) + ' ' + (answer.dependency !== null ? 'data-dependency="' + answer.dependency + '"' : '') + '">';
			...
			if (modal === null) {
				questions += '<h2>' + jsonQuests.modal[0] + '</h2>';
				questions += questBuild(0, jsonQuests.questions[0]);
			} else {
				questions += '<h2>' + jsonQuests.modal[0] + '</h2>';
				questions += questBuild(0, jsonQuests.questions[0]);
				$.each(modal, function(q, quest) {
					if(jsonQuests.questions[quest] !== undefined) {
						questions += '<h2>' + jsonQuests.modal[quest] + '</h2>';
						questions += questBuild(q, jsonQuests.questions[quest]);
					}
				});
